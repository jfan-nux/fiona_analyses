...
CREATE OR REPLACE TEMP TABLE markwu.raw_web_sessions_data_v2 AS (
SELECT 
platform 
, dd_device_id 
, dd_session_id 
...
CREATE OR REPLACE TABLE markwu.fortified_web_visitor_data_v2 AS (
WITH device_id AS (
SELECT 
dd_device_id 
, platform 
, MAX(consumer_id) AS consumer_id_logged_in_status 
FROM markwu.raw_web_sessions_data_v2
GROUP BY 1,2 
)
, joined AS (
SELECT 
...
CREATE OR REPLACE TABLE markwu.raw_unique_web_device_id_add_recent_28_days_web_info_v2 AS 
SELECT 
a.* 
...
FROM markwu.fortified_web_visitor_data_v2 a 
LEFT JOIN markwu.raw_web_sessions_data_v2 b 
...
CREATE OR REPLACE TABLE markwu.raw_unique_web_device_id_add_recent_session_web_info_v2 AS 
...
FROM markwu.raw_unique_web_device_id_add_recent_28_days_web_info_v2 a 
...
CREATE OR REPLACE TABLE markwu.logged_out_personalization_historical_web_device_id_v2 AS 
...
FROM markwu.raw_unique_web_device_id_add_recent_session_web_info_v2 a 
LEFT JOIN edw.finance.dimension_deliveries dd 
    ON dd.is_caviar != 1 
    AND dd.is_filtered_core = 1 
...
CREATE OR REPLACE TABLE markwu.logged_out_personalization_real_time_web_device_id_v2 AS 
WITH orders AS (
SELECT 
...
CREATE TABLE markwu.logged_out_personalization_training_v2 AS 
SELECT 
a.* 
...
FROM markwu.logged_out_personalization_real_time_web_device_id_v2 a 
LEFT JOIN markwu.logged_out_personalization_historical_web_device_id_v2 b 
    ON a.dd_device_id = b.dd_device_id 

;

GRANT SELECT ON markwu.logged_out_personalization_training_v2 TO ROLE read_only_users; 
