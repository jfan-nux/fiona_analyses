...
select * from fusion_dev.test_rahul_narakula.braze_sent_messages_push_bt 
where lower(canvas_name)=lower('TRG-NA-CartAbandonRx-REM-US-Other-NP-NA-NA-NA-PSH-ONLY');
select * from fusion_dev.test_rahul_narakula.braze_sent_messages_push_bt 
where canvas_id='65c6976b1f25d6004d059a73';
-- select * from edw.consumer.fact_consumer_notification_engagement where sent_at_date >= '2025-08-31'
-- and lower(canvas_name)=lower('TRG-NA-CartAbandonRx-REM-US-Other-NP-NA-NA-NA-PSH-ONLY');
select dispatch_id, count(1) cnt from fusion_dev.test_rahul_narakula.braze_sent_messages_push_bt 
    WHERE sent_at_timestamp_iso_utc >= '2025-08-20' group by all having cnt>1 limit 10; 


select message_id, count(1) cnt from edw.consumer.fact_consumer_notification_engagement 
where sent_at_date >= '2025-08-20' group by all having cnt>1 limit 10; 

select count(1) from fusion_dev.test_rahul_narakula.braze_sent_messages_push_bt
where   sent_at_timestamp_iso_utc >= '2025-08-20'
and dispatch_id  in (select distinct message_id from edw.consumer.fact_consumer_notification_engagement where sent_at_date >= '2025-08-20');

select * from edw.consumer.fact_consumer_notification_engagement where consumer_id='15730565' 
and sent_at_date >= '2025-08-20' and sent_at_date < '2025-08-23';

select * from fusion_dev.test_rahul_narakula.braze_sent_messages_push_bt where 1=1 and external_id='15730565'
and sent_at_timestamp_iso_utc >= '2025-08-20' and sent_at_timestamp_iso_utc < '2025-08-23' limit 10; 

select * from 
ext_braze.inbound.braze_canvas_flow_step_window limit 10;

select * from ext_braze.inbound.braze_canvas_flow_step_window where canvas_flow_step_api_id = 'd884a4f1-c960-44b7-bcbc-daf69c037263' limit 10;
select * from edw.growth.consumer_growth_accounting_scd3 where 1 =1 
and (lifestage ilike '%new%' or lifestage ilike '%non-purchaser%' or lifestage ilike '%churned%' or lifestage ilike '%dormant%' ) 
limit 10;
select distinct lifestage, lifestage_bucket from edw.growth.consumer_growth_accounting_scd3 ;
