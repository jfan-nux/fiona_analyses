SELECT DD_DEVICE_ID as device_id, consumer_id, iguazu_timestamp as event_ts,
'did_reonboarding' as did_reonboarding
from iguazu.consumer.M_onboarding_page_click_ice
WHERE convert_timezone('UTC','America/Los_Angeles', iguazu_timestamp) 
-- between '2025-10-01' and '2025-10-05'
...
create or replace table proddb.fionafan.did_reonboarding_2curie_debug as (
  ...
  WITH exposure AS (
    ...
    WHERE
      CAST(consumer_volume_curie.dte AS DATETIME) BETWEEN DATEADD(DAY, -28, '2025-09-08T20:04:00') AND CAST('2025-11-03T20:04:00' AS DATETIME)
  ), did_reonboarding AS (
    SELECT
      *
      FROM metrics_repo.public.did_reonboarding AS did_reonboarding
    WHERE
      CAST(did_reonboarding.event_ts AS DATETIME) BETWEEN CAST('2025-09-08T20:04:00' AS DATETIME) AND CAST('2025-11-03T20:04:00' AS DATETIME)
  ), consumer_volume_curie_ROLLING_28_1_DAY_True_exp_events /* Join exposures with events */ AS (
    SELECT
      exposure.*,
      consumer_volume_curie.*,
      did_reonboarding.did_reonboarding AS did_reonboarding_did_reonboarding
    FROM exposure
    LEFT JOIN consumer_volume_curie
      ON TO_CHAR(exposure.bucket_key) = TO_CHAR(consumer_volume_curie.device_id)
      AND consumer_volume_curie.dte BETWEEN DATEADD(DAY, -28, '2025-10-06') AND DATEADD(DAY, -1, '2025-10-06')
    /* join with dimensions if exist */
    LEFT JOIN did_reonboarding
      ON TO_CHAR(exposure.bucket_key) = TO_CHAR(did_reonboarding.device_id)
    WHERE
      1 = 1
    ...
    SELECT
      bucket_key,
      experiment_group,
      did_reonboarding_did_reonboarding AS did_reonboarding,
...
WITH metric_data AS (
  ...
  WITH exposure AS (
    ...
    FROM proddb.public.firefly_consumer_volume_curie_measure AS consumer_volume_curie
    WHERE
      CAST(consumer_volume_curie.dte AS DATE) BETWEEN DATEADD(DAY, -28, '2025-08-04') AND '2025-08-10'
  ), did_reonboarding AS (
    SELECT
      *
    FROM metrics_repo.public.did_reonboarding AS did_reonboarding
    WHERE
      CAST(did_reonboarding.event_ts AS DATE) BETWEEN '2025-08-04' AND '2025-08-10'
  ), consumer_volume_curie_ROLLING_28_1_DAY_True_exp_metrics /* Aggregate events and compute metrics */ AS (
    SELECT
      bucket_key,
      did_reonboarding,
    ...
    FROM (
      SELECT
        did_reonboarding.did_reonboarding,
  ...
  LEFT JOIN cuped_data_lag_7
    ...
    AND metric_data.did_reonboarding = cuped_data_lag_7.did_reonboarding
  WHERE
  ...
  FROM (
    WITH processed_dimension_data AS (
      ...
      SELECT
        'did_reonboarding' || '_' || COALESCE(TO_CHAR(did_reonboarding), 'NULL') AS variant_name,
