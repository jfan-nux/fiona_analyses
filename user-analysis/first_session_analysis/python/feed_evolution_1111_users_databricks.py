...
# MAGIC %md
# MAGIC # Feed Evolution Analysis for 1111 Activity Pattern Users
# MAGIC 
# MAGIC **Compares store embeddings across sessions to latest_week_4 final state**
# MAGIC 
# MAGIC ### Base Table
...
# MAGIC 1. Samples 50K users from each cohort (new/post_onboarding/active) with 1111 activity pattern
...
# MAGIC    - latest_week_1, latest_week_2, latest_week_3
# MAGIC    - latest_week_4 (used as final state reference)
# MAGIC 3. Generates BGE embeddings for store tags, categories, and names (by event type: impression, attribution, funnel_store)
# MAGIC 4. Calculates cosine similarity from each session to the latest_week_4 final state
...
# Load data with 1111 activity pattern
query = """
SELECT 
...
# MAGIC %md
# MAGIC ## 7. Load BGE Model and Generate Week 4 Embeddings
...
# MAGIC %md
# MAGIC ### Process Week 4 Embeddings by Store Type
...
def process_store_type(session_pd, store_type, tags_col, categories_col, names_col, 
                       session_type, model, week4_embeddings, FINAL_SESSION_TYPE):
    """
...
def process_session_type(session_type, session_dfs, STORE_TYPES, model, 
                        week4_embeddings, FINAL_SESSION_TYPE):
    """
...
# MAGIC FROM proddb.fionafan.feed_evolution_1111_cosine_similarity
