...
for cohort in ['new', 'active', 'post_onboarding']:
    ...
    for rank, (idx, row) in enumerate(cohort_data.iterrows(), 1):
        print(f"{rank:<6} {row['feature']:<45} {row['coefficient']:>9.4f} {row['pct_impact']:>11.1f}%")
...
# MAGIC %md
# MAGIC ### Reload Data and Define Features
...
def train_xgboost_model(
    ...
    Args:
        ...
        cohort_type: Cohort to filter for
    ...
    # Check available features (case-insensitive)
    df_columns_upper = [c.upper() for c in df_cohort.columns]
    column_name_map = {c.upper(): c for c in df_cohort.columns}
...
print(f"{'='*80}")
print(f"\n{'Cohort':<20} {'N Obs':<12} {'Positive %':<12} {'Test AUC':<12}")
print("-" * 60)
...
for result in [xgb_results_new, xgb_results_active, xgb_results_post]:
    ...
    print(f"{cohort:<20} {n_obs:<12,} {base_rate:<12.2f} {test_auc:<12.4f}")
...
try:
    ...
    for _, row in lr_results_pd.iterrows():
        lr_aucs[row['COHORT_TYPE']] = row['TEST_AUC']
...
# MAGIC %md
# MAGIC ### XGBoost Analysis Complete
...
print(f"   2. Logistic regression provides linear coefficients")
print(f"   3. Both approaches identify important predictors")
print(f"   4. Compare results to validate feature importance")
