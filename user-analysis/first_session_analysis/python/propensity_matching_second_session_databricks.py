...
# MAGIC %md
# MAGIC # Propensity Score Matching: Second Session Return Impact Analysis
# MAGIC
# MAGIC **Goal:** Find "identical twins" based on first session behavior - one who had a second session, one who didn't.
...
# MAGIC
# MAGIC **Method:**
# MAGIC 1. Build propensity score model to predict second session return based on first session features + week 1 orders
# MAGIC 2. Match treatment (had 2nd session) with control (didn't) based on similar propensity scores
# MAGIC 3. Match within cohort type for fair comparisons
# MAGIC 4. Calculate outcomes and compare between matched groups
...
MATCHING_FEATURES = [
    ...
    # Week 1 order performance features
    'WEEK1_HAD_ORDER',
    'WEEK1_ORDER_COUNT',
...
# MAGIC %md
# MAGIC ## Load First Session Data with Second Session and 1111 Cohort
...
had_second = df_first_session.filter(F.col("had_second_session") == 1).count()
no_second = df_first_session.filter(F.col("had_second_session") == 0).count()
week1_ordered = df_first_session.filter(F.col("week1_had_order") == 1).count()

print(f"\n✅ Loaded {total_count:,} first sessions")
print(f"   - Had second session: {had_second:,} ({100*had_second/total_count:.1f}%)")
print(f"   - No second session: {no_second:,} ({100*no_second/total_count:.1f}%)")
...
# Get propensity scores (probability of having second session)
...
print("      - Sample 20K from TREATMENT group")
print("      - Use control pool (up to 50K) for better match quality")

# Show distribution by cohort
cohort_dist = df_with_propensity.groupBy("cohort_type", "had_second_session").count().collect()

print(f"\n  📊 Full sample sizes by cohort:")
for row in sorted(cohort_dist, key=lambda x: (x['cohort_type'], -x['had_second_session'])):
    status = "Treatment" if row['had_second_session'] == 1 else "Control"
...
for cohort in ['new', 'active', 'post_onboarding']:
    ...
    treatment_full = cohort_data_full.filter(F.col("had_second_session") == 1)
    control_full = cohort_data_full.filter(F.col("had_second_session") == 0)
    ...
    # For CONTROL: use pool up to 50K
...
print(f"  Treatment (had 2nd session):  {treatment_1111_rate:.2%}")
print(f"  Control (no 2nd session):     {control_1111_rate:.2%}")
...
for idx, (title, treatment_col, control_col, unit) in enumerate(metrics):
    ...
    bars1 = ax.bar(x - width/2, treatment_display, width, label='Had 2nd Session', color='#2ca02c', alpha=0.8)
    bars2 = ax.bar(x + width/2, control_display, width, label='No 2nd Session', color='#d62728', alpha=0.8)
...
plt.suptitle('Propensity Matched Comparison: Second Session Impact', fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
display(fig)
...
bars1 = ax1.bar(x - width/2, treatment_vals, width, label='Had 2nd Session', color='#2ca02c', alpha=0.8)
bars2 = ax1.bar(x + width/2, control_vals, width, label='No 2nd Session', color='#d62728', alpha=0.8)
...
bars1 = ax2.bar(x - width/2, treatment_vals, width, label='Had 2nd Session', color='#2ca02c', alpha=0.8)
bars2 = ax2.bar(x + width/2, control_vals, width, label='No 2nd Session', color='#d62728', alpha=0.8)
...
bars1 = ax3.bar(x - width/2, treatment_vals, width, label='Had 2nd Session', color='#2ca02c', alpha=0.8)
bars2 = ax3.bar(x + width/2, control_vals, width, label='No 2nd Session', color='#d62728', alpha=0.8)
...
bars1 = ax4.bar(x - width/2, treatment_vals, width, label='Had 2nd Session', color='#2ca02c', alpha=0.8)
bars2 = ax4.bar(x + width/2, control_vals, width, label='No 2nd Session', color='#d62728', alpha=0.8)
...
output_table = "propensity_matched_second_session_results"
...
output_summary_table = "propensity_matched_second_session_summary"
...
# MAGIC %md
# MAGIC ## Conditional Analysis: Week 1 Ordering
...
# Analysis conditional on week 1 ordering
...
print(f"   - Treatment: Users who had a second session")
print(f"   - Control: Similar users (by first session + week 1 orders) who didn't")
print(f"\n💡 Key Findings:")
...
print(f"\n🎯 Interpretation:")
print(f"   This analysis shows the causal impact of having a second session")
print(f"   by comparing identical twins based on first session + week 1 order behavior")
