"""
Session Conversion Prediction with Transformer - WITH FUNNEL EVENTS (For Comparison)

⚠️  WARNING: This version includes funnel events which may cause data leakage!
Use transformer_attribution_no_funnel.py for unbiased attribution analysis.

This version is provided only for comparison to show the impact of including funnel events.
"""
...
def main():
    print("\n" + "="*80)
    print("⚠️  TRANSFORMER ATTRIBUTION - WITH FUNNEL EVENTS (May Have Data Leakage)")
    print("="*80)
    
    os.makedirs('outputs', exist_ok=True)
    os.makedirs('plots', exist_ok=True)
    
    # Load data - INCLUDES ALL EVENTS (including funnel)
    print("\n📊 Loading events (INCLUDING funnel events - may cause data leakage)...")
    ...
    print(f"✓ Loaded {len(df_events):,} events (including funnel events)")
    ...
    for epoch in range(1, 5):
        ...
        if val_res['auc'] > best_val_auc:
            ...
            torch.save(model.state_dict(), 'outputs/best_transformer_with_funnel.pt')
    ...
    model.load_state_dict(torch.load('outputs/best_transformer_with_funnel.pt'))
    
    print("\n🎯 Computing gradient × input attribution...")
    model.train()
    grad_attributions = []
    samples_processed = 0
    ...
    print(f"\n✓ Computed attribution for {len(df_grad_attribution):,} events")
    ...
    print("\n" + "="*80)
    print("TOP EVENTS BY ATTRIBUTION LIFT (WITH FUNNEL - LEAKAGE!)")
    print("="*80)
    for idx, (event_type, row) in enumerate(event_pivot.head(15).iterrows(), 1):
        marker = "⚠️ FUNNEL" if any(x in event_type.lower() for x in ['add', 'cart', 'checkout', 'place_order']) else ""
        print(f"{idx:2}. {event_type:40} | Lift: {row['attribution_lift']:+.4f} {marker}")
    
    # Save results
    df_grad_attribution.to_csv('outputs/gradient_attribution_with_funnel.csv', index=False)
    event_pivot.to_csv('outputs/event_type_attribution_with_funnel.csv')
    
    print(f"\n✅ Complete!")
    print(f"\nResults saved:")
    print(f"  - outputs/best_transformer_with_funnel.pt")
    print(f"  - outputs/gradient_attribution_with_funnel.csv")
    print(f"  - outputs/event_type_attribution_with_funnel.csv")
