...
# MAGIC %md
# MAGIC # Transformer Attribution - WITH FUNNEL EVENTS (For Comparison)
# MAGIC 
# MAGIC ⚠️  **WARNING: This version includes funnel events which cause data leakage!**
# MAGIC 
# MAGIC Use `transformer_attribution_no_funnel_pyspark.py` for unbiased analysis.
# MAGIC 
# MAGIC This version demonstrates the impact of including funnel events:
# MAGIC - Likely to achieve AUC > 0.95 due to leakage
# MAGIC - Attention will focus on funnel events (add_item, checkout, etc.)
# MAGIC - Comparison baseline only
...
# MAGIC %md
# MAGIC ## Load Data - INCLUDING ALL EVENTS

# COMMAND ----------

print("⚠️  Loading events (INCLUDING funnel events - expect data leakage)...")
...
WHERE e.EVENT_TYPE NOT ILIKE '%error%' AND e.EVENT NOT ILIKE '%error%'
  -- KEEP ALL EVENTS INCLUDING FUNNEL (this will cause data leakage)
"""
...
print(f"✓ Loaded {event_count:,} events (including funnel events)")
...
print(f"⚠️  High AUC likely due to data leakage from funnel events!")
...
if len(df_grad_attribution) > 0:
    print(f"✓ Columns in attribution data: {df_grad_attribution.columns.tolist()}")
    print(f"✓ Sample data:")
    print(df_grad_attribution.head(3))
else:
    print("⚠️  WARNING: No attribution data captured! Check if x.grad is being populated.")
    print("This likely means x.retain_grad() didn't work or no gradients flowed.")
...
if len(df_grad_attribution) == 0:
    print("⚠️  ERROR: No attribution data captured!")
    print("💡 Gradients were not populated. This is a CRITICAL ERROR.")
    print("   SOLUTION: You MUST restart the Databricks kernel and re-run ALL cells from the beginning.")
    print("   The notebook is running OLD CACHED CODE. Simply re-running this cell won't work.")
    raise RuntimeError("No attribution data - restart notebook!")
elif 'event_type' not in df_grad_attribution.columns:
    print("⚠️  ERROR: Attribution data is missing event_type column!")
    print(f"Available columns: {df_grad_attribution.columns.tolist()}")
    print("💡 SOLUTION: You MUST restart the Databricks kernel and re-run ALL cells.")
    print("   The notebook is running OLD CACHED CODE from before the fix.")
    raise RuntimeError("Missing event_type column - restart notebook!")
else:
    print(f"✅ Attribution data has all required columns: {df_grad_attribution.columns.tolist()}")
...
print("\n⚠️  Note: In WITH FUNNEL version, funnel events will dominate (data leakage)")
...
axes[0, 0].set_title('**Top Events by Attribution Lift (WITH Funnel - Leakage!)**', fontsize=12, fontweight='bold')
...
event_importance_spark.write.format("snowflake") \
    .options(**OPTIONS) \
    .option("dbtable", "proddb.fionafan.transformer_event_attribution_with_funnel") \
    .mode("overwrite") \
    .save()

print("✓ Saved event attribution to proddb.fionafan.transformer_event_attribution_with_funnel")
...
pos_importance_spark.write.format("snowflake") \
    .options(**OPTIONS) \
    .option("dbtable", "proddb.fionafan.transformer_position_attribution_with_funnel") \
    .mode("overwrite") \
    .save()

print("✓ Saved position attribution to proddb.fionafan.transformer_position_attribution_with_funnel")
...
store_importance_spark.write.format("snowflake") \
    .options(**OPTIONS) \
    .option("dbtable", "proddb.fionafan.transformer_store_attribution_with_funnel") \
    .mode("overwrite") \
    .save()

print("✓ Saved store attribution to proddb.fionafan.transformer_store_attribution_with_funnel")
...
print(f"  ⚠️  HIGH AUC due to data leakage from funnel events!")

print(f"\n**Top 5 Most Important Events (expect funnel events to dominate):**")
for idx, (event_type, row) in enumerate(event_pivot.head(5).iterrows(), 1):
    # Mark funnel events
    marker = "⚠️ FUNNEL EVENT" if any(x in event_type.lower() for x in ['add', 'cart', 'checkout', 'place_order']) else ""
    print(f"  {idx}. {event_type:40} | Lift: {row['attribution_lift']:+.4f} {marker}")
...
print(f"  - proddb.fionafan.transformer_event_attribution_with_funnel")
print(f"  - proddb.fionafan.transformer_position_attribution_with_funnel")
print(f"  - proddb.fionafan.transformer_store_attribution_with_funnel")

print(f"\n📝 **NEXT STEP**: Compare with NO FUNNEL version to see impact of data leakage!")
