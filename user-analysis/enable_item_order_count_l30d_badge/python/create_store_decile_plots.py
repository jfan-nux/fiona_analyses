...
def main():
    ...
    for i, (metric, label) in enumerate(zip(metrics, metric_labels)):
        ...
        # Set x-axis labels - show all deciles (1-10)
    ...
    plot_filename = plots_dir / f'store_decile_analysis_plots_with_volume_{timestamp}.png'
    ...
    # Print detailed decile breakdown (show all 10 deciles)
    print('\n📊 Performance by Store Volume Decile (D1 to D10):')
    for decile in unique_deciles:
    ...
    for treatment in df_filtered['treatment_arm'].unique():
        ...
        total_deciles = len(treatment_data)
        
        print(f'     Positive Effects: {positive_order_deciles}/{total_deciles} deciles (orders), {positive_revenue_deciles}/{total_deciles} deciles (revenue)')
    ...
    for treatment in df_filtered['treatment_arm'].unique():
        ...
        # Identify sweet spots (above average effects)
        ...
        if not high_effect_deciles.empty:
            ...
            print(f'     Sweet Spot Deciles (Above Average): {", ".join(sweet_spot_labels)}')
    
    # Store volume context (show all deciles)
    print(f'\n📋 Store Volume Context (All Deciles):')
    print('-' * 50)
    for decile in unique_deciles:
        decile_data = df_filtered[df_filtered['volume_decile'] == decile]
        if not decile_data.empty:
